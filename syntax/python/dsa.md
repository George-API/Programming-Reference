...
- [0. Complexity Cheatsheet (common ops)](#0-complexity-cheatsheet-common-ops)
- [1. "DSA Python" syntax patterns (dicts/sets/sort/bisect/heap)](#1-dsa-python-syntax-patterns-dictssetssortbisectheap)
- [2. Arrays / Strings — core patterns](#2-arrays--strings--core-patterns)
- [3. Stack Patterns](#3-stack-patterns)
- [4. Heap Patterns](#4-heap-patterns)
- [5. Binary Search Templates](#5-binary-search-templates)
- [6. Graph Patterns (BFS / DFS)](#6-graph-patterns-bfs--dfs)
- [7. Union-Find (Disjoint Set Union)](#7-union-find-disjoint-set-union)
- [8. Intervals (merge / sweep)](#8-intervals-merge--sweep)
- [9. DP Patterns (Intermediate)](#9-dp-patterns-intermediate)
- [10. Greedy: common "proof-y" rules of thumb](#10-greedy-common-proof-y-rules-of-thumb)
- [11. Common Bug Traps (read this before submitting)](#11-common-bug-traps-read-this-before-submitting)
...
**INVARIANT**: l and r bound the remaining candidate region
...
**INVARIANT**: window [l..r] is ALWAYS valid after the while-loop
...
**INVARIANT**: mp[p] = how many times we've seen prefix sum p so far
...
**WHEN**: many "add val to [l..r]" updates, then build final array once

**INVARIANT**: diff stores boundary deltas; prefix over diff reconstructs values
...
**WHEN**: "next greater/smaller to the right/left"

**INVARIANT**: stack keeps indices with monotonic property on values (here: decreasing stack; top is smallest among stacked?)
...
**WHEN**: matching pairs, nested structure validation

**INVARIANT**: stack contains opening brackets not yet closed
...
**WHEN**: you want k largest items, stream-friendly

**INVARIANT**: heap always holds current best k items (smallest at top)
...
**WHEN**: merge many sorted sources efficiently

**INVARIANT**: heap stores the current "frontier" element of each list
...
**WHEN**: classic search on sorted list
...
**INVARIANT**: you shrink [lo, hi] to the first True (or last True) boundary
...
**WHEN**: shortest number of edges / moves

**INVARIANT**: first time you visit a node is the shortest distance
...
**WHEN**: connected components, cycle detection (with extra state), topological sorts

**INVARIANT**: "seen" ensures each node processed once
...
**WHEN**: union of intervals, simplify schedule

**INVARIANT**: output stays merged and sorted
...
**WHEN**: maximum concurrent intervals, "how many active at once"

**IMPORTANT**: choose inclusive/exclusive: [s, e) is usually simplest
...
**WHEN**: choose non-adjacent items for max value
...
**WHEN**: each item used at most once

**INVARIANT**: iterate capacity backwards to avoid reusing same item twice
...
**WHEN**: state transitions are clear, recursion simplifies logic

**NOTE**: ensure state space is not huge or you may TLE/MLE
...
- **Intervals**: Sort by end time to maximize number of non-overlapping intervals.
- **Scheduling**: Sort tasks, maintain a heap of active tasks by "worst" candidate to drop.
- **Always ask**: "What is the local choice, and why can't it block the global optimum?"
...
- **Sliding window**: After while-loop ends, window MUST be valid (your invariant).
- **Hash maps**: Delete keys when freq hits 0 if you rely on len(freq).
- **Recursion depth**: Python recursion can crash on large graphs; prefer iterative BFS/DFS.
- **Using list for seen**: membership is O(n); use set/dict for O(1) average.
