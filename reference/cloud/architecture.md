...
- [0. Platform Engineering Add-ons](#0-platform-engineering-add-ons)
- [1. Identity & Auth (Advanced)](#1-identity--auth-advanced)
- [2. Networking & Edge (Advanced)](#2-networking--edge-advanced)
- [3. API Platform & Governance (Advanced)](#3-api-platform--governance-advanced)
- [4. Messaging Guarantees & Workflow Patterns (Advanced)](#4-messaging-guarantees--workflow-patterns-advanced)
- [5. Data Contracts, Schema & Governance (Advanced)](#5-data-contracts-schema--governance-advanced)
- [6. Multi-tenant SaaS Patterns (Advanced but practical)](#6-multi-tenant-saas-patterns-advanced-but-practical)
- [7. Reliability Engineering (SRE-level)](#7-reliability-engineering-sre-level)
- [8. Secrets, Keys, and Confidential Options](#8-secrets-keys-and-confidential-options)
- [9. Operational Excellence (Enterprise)](#9-operational-excellence-enterprise)
- [10. Advanced "Reference Blueprints"](#10-advanced-reference-blueprints)
...
- Separate inbound, app, and data subnets; NSGs as defense-in-depth (even with cloud services)

### Multi-region connectivity

- **Virtual WAN** for branch connectivity; hub-spoke for simpler estates

> **Troubleshooting**: For networking troubleshooting (DNS, routing, private endpoints), see [OSI Model - Layer 3](cloud_osi.md#layer-3--network-ip-routing).
...
## 5. Data Contracts & Schema (Application Layer)

> **Note**: For comprehensive data management concepts (operating model, quality systems, integrity patterns), see [Data Management Best Practices](data_management.md).

### Application-level data contracts

- **API contracts**: OpenAPI/Swagger schemas, versioning strategy (URL/header/query param)
- **Event contracts**: CloudEvents schema, backward-compatible evolution
- **Message schemas**: Avro/Protobuf/JSON schemas for Service Bus/Event Grid

### Schema versioning (application patterns)

- Backward compatible changes as default; breaking changes require new version/topic
- Schema registry (Azure Schema Registry) for Avro/JSON schemas
- Contract testing in CI/CD pipelines

### Storage formats (application data)

- **Application databases**: SQL (OLTP), Cosmos DB (document/NoSQL)
- **Caching**: Redis for session state, frequently accessed data
- **File storage**: Blob Storage for documents, images, exports
...
> **Troubleshooting**: For transport-layer troubleshooting (timeouts, connection pooling, retries), see [OSI Model - Layer 4](cloud_osi.md#layer-4--transport-tcpudp-ports-reliability).
...
> **Note**: If you want, tell me which scenario you're targeting (API platform, integration hub, analytics pipeline, or SaaS multi-tenant) and this can be condensed into a single "canonical design" with minimal service set, security posture (private endpoints vs public edge), SLOs + core alerts, and data + event contract checklist.
